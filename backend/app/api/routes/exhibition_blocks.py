...
async def add_exhibition_block(
    ...
    try:
        block = await exhibition_block_crud.create_exhibition_block(
            session=session,
            block_in=block_in,
            items=block_in.items,
        )
...
async def delete_exhibition_block(exhibition: ExhibitionOr404, block_id: UUID, session: SessionDep):
    try:
