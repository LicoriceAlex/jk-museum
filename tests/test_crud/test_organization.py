...
async def test_organization(db_session: AsyncSession) -> Organization:
    org_data = OrganizationCreate(name=TEST_ORG_NAME, email=TEST_ORG_EMAIL)
...
async def test_duplicate_organization_email(db_session: AsyncSession, test_organization):
    duplicate_org_data = OrganizationCreate(name="Duplicate Org", email=TEST_ORG_EMAIL)
...
async def test_update_organization(db_session: AsyncSession, test_organization):
    update_data = OrganizationCreate(name=UPDATED_ORG_NAME, email=UPDATED_ORG_EMAIL)

    updated_org = await organization_crud.update_organization(
        db_session, test_organization, update_data
    )
...
async def test_get_organizations(db_session: AsyncSession, test_organization):
    org_data_2 = OrganizationCreate(name=TEST_2_ORG_NAME, email=TEST_2_ORG_EMAIL)
